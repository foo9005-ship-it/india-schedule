"""
[vendored] このファイルはClaude CodeのdocxスキルVendor(/mnt/skills/public/docx/scripts/)由来。
generate_minutes.pyがLambda等、docxスキルが存在しない環境でも動作できるよう同梱している。
office/helpers/__init__.py も同スキルから同梱済み（標準ライブラリのみに依存）。
更新する場合は元のスキル側と手動で同期すること。
